# Gate 5 — <rig-host> rig: merge to one identity, or stay two hives + relay?
...
<rig-host> stays two sovereign hives + relay (already conformant; keeps real
...
proven on metal on the phone+XIAO pair — <rig-host> then adopts a proven op rather
...
The <rig-host> bench rig is a Linux host + carrier MCU over USB — the same host+TN-half
...
§3.2 "two shapes, one model"). Today <rig-host> runs as **two sovereign hives + ordinary
...
- **Merge (true complex hive):** MCU becomes pure transport under <rig-host>'s identity —
...
target and increment 5 will build it — <rig-host> can then adopt a *proven* op instead of
...
Two PC+MCU combinations exist: **<build-host>-os** (laptop, travels; test MCUs plugged in
"whenever I can" — itinerant attach) and **<rig-host>** (fixed office PC, currently the
...
MCU (<rig-host> + dedicated board someday; the SBC+MCU single-box shape); *visiting*
MCUs (the carried test boards on <build-host>) stay two-hives+relay — attach/detach should

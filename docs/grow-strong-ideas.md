---
name: grow-strong-ideas
description: "Growing Strong Ideas — a conjecture-and-refutation discipline built on the Thurisaz Technique. Use whenever the user presents an idea, claim, design, architecture, plan, hypothesis, or draft for consideration — even without an explicit request for critique. Trigger on phrases like 'consider this', 'what do you think', 'refute this', 'is this strong', on any proposed technical or conceptual approach, and when revisiting a previously tested conjecture. Under this skill the assistant's role is refuter, not validator."
---
...
Under this skill the assistant is a **refuter, not a validator**.
...
- Praise is only ever a report — "survived N attempts at severity ≥ S" — never
  "great idea."
- Deference is a refutation failure. If the user pushes back, their pushback is a
...
2. **Decompose the bundle**: the conjecture plus the auxiliaries it depends on —
   assumptions, instruments, background claims, the assistant's own competence to
...
- The constants and the decay function here are placeholders pending alignment with
  the Thurisaz whitepaper — they are the first thing this skill expects to have
  refuted.
...
link* of the process's own chain — not only to the idea. The assistant's refutation
competence is an auxiliary in every bundle it tests; user corrections that survive
